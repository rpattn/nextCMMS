...
export default function EditWorkOrderModal({ id, open, onClose, onSaved }: { id: string; open: boolean; onClose: () => void; onSaved?: () => void }) {
    ...
    (async () => {
      try {
        ...
        const nextForm: WorkOrder = {
          ...
          estimatedStartDate: data.estimatedStartDate ? (data.estimatedStartDate as string).substring(0, 10) : '',
        ...
        setForm(nextForm);
        setPrimaryUser(nextForm.primaryUser ? { id: nextForm.primaryUser.id, label: nextForm.primaryUser.name || `${nextForm.primaryUser.firstName || ''} ${nextForm.primaryUser.lastName || ''}`.trim() || `#${nextForm.primaryUser.id}` } : null);
  ...
  const submit = async () => {
    ...
    try {
      ...
      payload.estimatedStartDate = form.estimatedStartDate || null;
      payload.estimatedDuration = form.estimatedDuration != null ? Number(form.estimatedDuration) : null;
      ...
      <DialogContent>
        {!form ? (
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField label={t('estimated_start_date') || 'Estimated Start'} type="date" value={(form.estimatedStartDate as string) || ''} onChange={(e) => onChange({ estimatedStartDate: e.target.value })} fullWidth InputLabelProps={{ shrink: true }} />
            <TextField label={t('estimated_duration') || 'Estimated Duration (h)'} type="number" inputProps={{ step: '0.25', min: '0' }} value={(form.estimatedDuration as any) ?? ''} onChange={(e) => onChange({ estimatedDuration: e.target.value ? Number(e.target.value) : null })} fullWidth />
